...
function M.on_attach(client, buffer)
	...
	self:map("<leader>le", vim.diagnostic.open_float, { desc = "Line Diagnostics" })
	...
	self:map("]D", M.diagnostic_goto(true, "ERROR"), { desc = "Next Error" })
	self:map("[D", M.diagnostic_goto(false, "ERROR"), { desc = "Prev Error" })
	self:map("]w", M.diagnostic_goto(true, "WARNING"), { desc = "Next Warning" })
	self:map("[w", M.diagnostic_goto(false, "WARNING"), { desc = "Prev Warning" })
	self:map("<leader>la", vim.lsp.buf.code_action, { desc = "Code Action", mode = { "n", "v" }, has = "codeAction" })

	local format = Vreq("plugins.configs.lsp-config.format").format
